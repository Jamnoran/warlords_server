...
public class Spell {
	...
	private boolean checkTargetTypeIsCorrect() {
		...
		CommunicationUtil.sendRotateTargetResponse(rotationResponse, getGameServer());
...
//		getGameServer().sendGameStatus();
	...
	public void damageMinion(Minion minion, Amount damageAmount, float penetration, String damageType) {
		...
		CommunicationUtil.sendCombatText(new CombatTextResponse(false, minion.getId(), "" + totalDamageAfterReduction, damageAmount.isCrit(), GameUtil.COLOR_DAMAGE), gameServer);
	...
	public void healHero(Integer heroId, Amount amount) {
		...
		CommunicationUtil.sendCombatText(new CombatTextResponse(true, hero.getId(), "" + amount.getAmount(), amount.isCrit(), GameUtil.COLOR_HEAL), gameServer);
	}
